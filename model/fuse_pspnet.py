...
from model.pspnet import PPM, PPM_v2, channel_attention
...
class Attention_v3_FusedPSPNet(nn.Module):
    def __init__(self, layers=50, bins=(1, 2, 3, 6), dropout=0.1, classes=2, zoom_factor=8, use_ppm=True,
                 criterion=nn.CrossEntropyLoss(ignore_index=255), pretrained=True):
        super(Attention_v3_FusedPSPNet, self).__init__()
        assert layers in [50, 101, 152]
        assert 2048 % len(bins) == 0
        assert classes > 1
        assert zoom_factor in [1, 2, 4, 8]
        ...
        fea_dim = 2048
        if use_ppm:
            self.ppm = PPM_v2(fea_dim, int(fea_dim / len(bins)), bins)
            fea_dim *= 3
        self.cls = nn.Sequential(
            nn.Conv2d(fea_dim, 512, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(512),
